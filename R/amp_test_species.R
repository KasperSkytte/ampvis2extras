...
#'
#' @usage amp_test_species(data, group_by)
#'
#' @param data (\emph{required}) Data list as loaded with \code{amp_load()}.
#' @param group_by (required) The group to test.
...
#' results <- amp_test_species(AalborgWWTPs, group_by = "Plant")
#'
#' #Show plots
#' results$plot_sig
#' results$plot_MA
...
amp_test_species <- function(data,
  ...
  p2 <- ggplot(data = point_df, aes_string(x = "Tax", y = "Abundance", color = group_by)) +
